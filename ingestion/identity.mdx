...
`distinct_id` is the resolution key for all attribution. Getting identity right is the most consequential part of the integration — mismanaging it either fragments one user across many profiles (under-merged) or collides several users into one (over-merged).
