...
<Steps>
  ...
  <Step title="Connect PostgreSQL in Actioneer">
    ...
    | **SSH Tunnel** | Optional — enable to connect via a bastion host for private databases. Also required if your provider enforces SSL (e.g., some RDS configs, Heroku Postgres). |
...
<AccordionGroup>
  ...
  <Accordion title="When should I use the SSH Tunnel option?">
    Enable the SSH Tunnel if your PostgreSQL instance is in a private network (e.g., an RDS instance in a private subnet with no public endpoint), or if your provider requires SSL connections. The tunnel connects Actioneer via a bastion host that has access to the private network.
  </Accordion>
