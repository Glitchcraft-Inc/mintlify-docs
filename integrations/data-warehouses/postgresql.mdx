...
<AccordionGroup>
  ...
  <Accordion title="When should I use the SSH Tunnel option?">
    Enable the SSH Tunnel if your PostgreSQL instance is in a private network (e.g., an RDS instance in a private subnet with no public endpoint). The tunnel routes Actioneer's connection through a bastion host that has network access to your database. SSH tunneling is not the same as SSL — all Actioneer connections use TLS encryption regardless of whether the tunnel is enabled.
  </Accordion>
