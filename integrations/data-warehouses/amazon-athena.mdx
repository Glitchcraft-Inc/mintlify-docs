...
<Steps>
  ...
  <Step title="Create a dedicated IAM user with Athena permissions">
    ...
    <Accordion title="Quick setup with managed policies">
      If you prefer a faster setup, attach these three AWS managed policies to the IAM user instead of creating the custom policy above:

      - `AmazonAthenaFullAccess`
      - `AmazonS3FullAccess`
      - `AWSGlueConsoleFullAccess`

      These policies provide broader access than the custom policy. For tighter control over what Actioneer can access — especially in live environments — use the custom policy in the previous step.
    </Accordion>
  </Step>
