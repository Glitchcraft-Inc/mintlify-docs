...
| **Actioneer IP** | `35.244.14.238` (whitelist if your instance uses an IP allowlist) |
...
<Steps>
  <Step title="Locate your ClickHouse connection details">
    ...
    **Self-hosted:** Use the hostname or IP address of your ClickHouse server. The default HTTP port is `8123`. The default native port is `9000`.

    | Value | Location | Example |
    | :--- | :--- | :--- |
    | **Host** | Service overview or server config | `my-instance.clickhouse.cloud` |
    | **Port** | Service overview or server config | `8123` |
  </Step>
