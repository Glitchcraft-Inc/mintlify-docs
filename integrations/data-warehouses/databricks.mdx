...
| &nbsp; `SELECT ON ALL TABLES IN SCHEMA` | &nbsp; All tables and views in schema | &nbsp; Read data from tables and views |
    ...
    <Note>
      These commands follow the Unity Catalog privilege model. `SELECT ON SCHEMA` is not a valid Unity Catalog grant — you must grant `SELECT` on individual tables, views, or use `ALL TABLES IN SCHEMA` as shown above. If you are unsure about your catalog or schema names, forward this page to your Databricks admin.
    </Note>
  </Step>
